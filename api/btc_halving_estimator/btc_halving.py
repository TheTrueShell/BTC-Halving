...
def get_latest_block_data():
    ...
    try:
        response = requests.get(f"{BLOCKCHAIR_API_URL}?limit=1")
...
def get_average_block_time(current_block_height):
    ...
    num_blocks = 100 if blocks_to_halving > 10000 else 50
    ...
    if data and len(data) > 1:
        return get_weighted_block_time(data)
    logging.warning("Could not calculate average block time")
    return 600  # fallback to 10 minutes


def get_weighted_block_time(data):
    total_weight = 0
    total_time = 0
...
def estimate_next_halving_date():
    if latest_block_data := get_latest_block_data():
...
if __name__ == "__main__":
    if next_halving_date := estimate_next_halving_date():
